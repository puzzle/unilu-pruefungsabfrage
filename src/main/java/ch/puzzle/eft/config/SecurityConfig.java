...
public class SecurityConfig {
        ...
        return token -> new UserDetails() {
            ...
            public String getPassword() {
                if (token.getCredentials() == null) {
                    logger.debug("no password is set");
                    return "n/a";
                } else {
                    String credentials = token.getCredentials()
                                              .toString();
                    logger.debug("current password is {}",
                                 credentials.charAt(0) + "*****" + credentials.charAt(credentials.length() - 1));
                    return credentials;
                }
            }
    ...
    protected SecurityFilterChain configure(HttpSecurity http, AuthenticationManager authenticationManager, RequestAttributeAuthenticationFilter authenticationFilter) throws Exception {
        ...
        http.cors(Customizer.withDefaults())
            .authorizeHttpRequests(authorize -> authorize.requestMatchers("/eft/**") // pre-authenticated by Shibboleth
                                                         .authenticated()
                                                         .requestMatchers(homeUrl)
                                                         .permitAll()
                                                         .requestMatchers(errorUrl)
                                                         .permitAll()
                                                         .anyRequest()
                                                         .denyAll())
